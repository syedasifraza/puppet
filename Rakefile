...
task :bootstrap do
  ...
  commands = <<BOOTSTRAP
hostname #{hostname} && echo 127.0.0.1 testhostname >/etc/hosts && cd /etc/yum.repos.d/ && rpm -ivh http://yum.puppetlabs.com/puppetlabs-release-el-6.noarch.rpm && rpm -Uvh http://download.fedoraproject.org/pub/epel/6/i386/epel-release-6-8.noarch.rpm && yum install -y git puppet && rm -rf /etc/puppet/* && git clone #{REPO} /etc/puppet/ && puppet apply --modulepath=/etc/puppet/modules /etc/puppet/manifests/site.pp BOOTSTRAP
